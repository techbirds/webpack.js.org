...
告知 webpack 当选择模块 id 时需要使用哪种算法。将 `optimization.chunkIds` 设置为 `false` 会告知 webpack 没有任何内置的算法会被使用，但自定义的算法会由插件提供。`optimization.chunkIds` 有几个默认值：
...
默认 `optimization.concatenateModules` 在 `production` [模式](/configuration/mode/) 下被启用，而在其它情况下被禁用。
...
>>>>>>> 10c3952cf28c79aac42cef4dbd9ce884a1320f48
...
## optimization.providedExportsoptimizationprovidedexports$
...
在处理静态资源后添加额外的哈希编译，以获得正确的资产内容哈希。如果 `realContentHash` 设置为 `false`，则使用内部数据来计算哈希，当静态资源相同时，它可以更改。
...
=======
当这些模块已经包含在所有父模块中时，告诉 webpack 从 chunk 中检测和删除模块。设置的优化。removeAvailableModules ' to ' true '将启用此优化。默认在[' production ' mode](/configuration/mode/)中启用。
...
值 `'single'` 将创建一个运行时文件，供所有生成的 chunk 共享。此设置是以下命令的别名：
